...
const { Error: ErrorMongoose } = mongoose
const { ValidationError } = ErrorMongoose
...
export const handleError = (err, req, res, next) => {
  ...
  if (
    err instanceof ErrorMongoose &&
    !(err instanceof ValidationError || err.name === 'CastError')
  ) {
    statusCode = 500
    error = { message, name }
    ...
    errors = Object.assign(
      ...
      ...Object.keys(errors).map((k) => {
        const { message, name, value, kind, path } = errors[k]
        return {
          [k]: {
            message,
            name,
  ...
  } else if (err instanceof AplicationError || err.name === 'CastError') {
